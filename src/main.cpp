...
#if defined(NDEBUG)
# error "Rupinote cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x12918cb928251de14d03b757c808939ecbfc369c0dece68b7506db72d30052f4");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Rupinote: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Rupinote Signed Message:\n";
...
{
    // Rupinote: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Rupinote: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 60; // Rupinote: 10 minutes
static const int64 nTargetSpacing = 5 * 60; // Rupinote: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Rupinote: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Rupinote: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x12918cb928251de14d03b757c808939ecbfc369c0dece68b7506db72d30052f4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "RUPINOTE future of all";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("404044288a56f2707075d5fe6e0cc324972b26c486340404s2270bfae20c070a5ff461klsqwert1c72433170707c51b31dc62cd399d140404a545ajskf57370404") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1521402051;
        ...
        assert(block.hashMerkleRoot == uint256("0xd1d9c613b62f550282b3a3b441331c13c3188cb3f623736f57d162d0c7465af4"));
...
unsigned char pchMessageStart[4] = { 0x5f, 0xb0, 0xbf, 0x2b }; // Rupinote: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RupinoteMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RupinoteMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RupinoteMiner(CWallet *pwallet)
{
    printf("RupinoteMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("rupinote-miner");
    ...
    try { loop {
        ...
        printf("Running RupinoteMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RupinoteMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RupinoteMiner, pwallet));
}
